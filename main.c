...
int main() {
    ...
    for (int i = 0; i < PELATES; i++) {
        ...
        if ((pids[i] = fork()) == 0) { // Δημιουργία νέου process για κάθε πελάτη (παιδί)
            close(ptc[i][1]); // Κλείνουμε την εγγραφή στο pipe γονέα -> παιδί στο παιδί (1 = εγγραφή)
            close(ctp[i][0]); // Κλείνουμε την ανάγνωση στο pipe παιδιού -> γονέα στο παιδί (0 = ανάγνωση)
